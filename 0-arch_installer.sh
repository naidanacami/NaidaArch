...
case $formatdisk in
    y|Y|yes|Yes|YES)
        ...
        sgdisk -n 3::-0 --typecode=3:8e00 --change-name=3:'LVMROOT' ${disk} # partition 3 (lvm)
        ...
        if [[ ${disk} =~ "nvme" ]]; then
            ...
            echo -n "${luks_password}" | cryptsetup open ${disk}p3 cryptLVM -                   # open luks container
            #LVM for LVMROOT
            pvcreate /dev/mapper/cryptLVM                                                       # To create a PV
            vgcreate ${vg_name} /dev/mapper/cryptLVM
            lvcreate -L ${rootsize}G ${vg_name} -n root                                         # Create root
            lvcreate -l 100%FREE ${vg_name} -n home                                             # Create home
            # now format that container
            mkfs.ext4 /dev/${vg_name}/root                                                      # Format root ext4
            mkfs.ext4 /dev/${vg_name}/home                                                      # Format home ext4
            ...
            echo -n "${luks_password}" | cryptsetup open ${disk}3 cryptLVM -                    # open luks container
            #LVM for LVMROOT
            pvcreate /dev/mapper/cryptLVM                                                       # To create a PV
            vgcreate ${vg_name} /dev/mapper/cryptLVM
            lvcreate -L ${rootsize}G ${vg_name} -n root                                         # Create root
        ...
        mount /dev/${vg_name}/root /mnt                     # Moutning Root
        mkdir /mnt/home
        mount /dev/${vg_name}/home /mnt/home                # Mounting Home
        mkdir /mnt/boot
        #! mkdir /mnt/boot/efi
